...
import {
  CloseConfirm,
  TransitionScreen,
} from "@/app/plaground/home/verses-list/play/components/closeConfirm";
...
export default function PlayPage() {
  ...
  const onAnswer = () => {
    ...
    if (currentQuestion && currentQuestion.isCorrect) {
      ...
      if (totalQuestions > 0) {
        const result = Math.round(((currentIndex + 1) / totalQuestions) * 100);
      ...
      playSound({
        path: pickRandom([
          SoundList.sellectWin1,
          SoundList.sellectWin2,
          SoundList.sellectWin3,
        ]),
      });
      setIsWin(true);
    } else {
  ...
  if (currentQuestion) {
    return (
      <div
        className={
          "px-[20px] py-[20px] flex flex-col gap-[30px] relative mb-32 max-w-xl mx-auto"
        }
      >
        ...
        >
          {inFlash && (
            <Lottie animationData={flashLotie} loop={true} autoplay={true} />
          )}
          {inStar && (
            <Lottie
        ...
        <PlayInfo
          ...
          isOldError={
            isHard
              ? currentQuestion.missedCount > 0 && !hardSelected
              : currentQuestion.missedCount > 0 && !selectedVertset
          }
        ...
        <EndStateScreen
          correctVerset={`${
            bookMapById.get(currentQuestion.target.book_num)?.label
          } ${normalizeVersetTitle(currentQuestion.target)}`}
        ...
        {isEnded && <EndPartyScreen score={10 - wrongCount} />}
      </div>
    );
  }
}
...
const HeartCount: React.FC<HeartCountProps> = ({ count }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="text-red-500 text-xl font-bold font-['Feather']">
        {count}
      </div>
    </div>
  );
...
const ProgressBar: React.FC<{ progress: number; withEffect: boolean }> = ({
  progress,
  withEffect,
}) => {
  return (
...
const PlayInfo: React.FC<{
  ...
  return (
    <div className={"flex flex-col gap-[10px]"}>
      ...
      <div className="text-white/80 text-base font-normal leading-[30px]">
        {verset.content}
      </div>
    </div>
  );
...
const PlayConfirm: React.FC<{
  ...
  hardInfos: SimpleBook;
  mode?: Difficult;
}> = ({ selectedVerset, onConfirm, mode, hardInfos }) => {
...
const PlayPlayground: React.FC<{
  onSelectVerset: (v: Verset) => void;
  verses: Verset[];
  selectedVerse?: Verset;
}> = ({ onSelectVerset, verses, selectedVerse }) => {
      ...
      <AnimatePresence>
        {verses.map((verse) => (
          ...
          >
            ...
            >
              <span>
                {bookMapById.get(verse.book_num)?.label}{" "}
                {normalizeVersetTitle(verse)}
              </span>
              <motion.div
                className="text-5xl"
                initial={{ scale: 2 }}
                animate={{
                  scale: selectedVerse?.id === verse.id ? [2, 2.2, 2] : 2,
                }}
                transition={{ duration: 0.3 }}
              >
...
const EndStateScreen: React.FC<{
  ...
  return (
    <>
      {isOpened && (
        <TransitionScreen>
          <div className={"w-full h-fit py-8 pt-6 px-5 bg-[#213037]"}>
            <div className="max-w-xl mx-auto w-full  gap-4 flex flex-col justify-between">
              <div className={"flex items-center gap-[11px]"}>
                <DFlagIcon
                  className={cn(isError ? "text-[#CE5D55]" : "text-[#92d233]")}
                />
                <div
                  className={cn(
              ...
              {isError && (
                <div className={"flex justify-between items-center"}>
                  <div className={"flex flex-col gap-1"}>
                    <div className="text-[#92d233] text-sm font-normal leading-snug">
                      {missedAnswerMessage} :
                    </div>
                    <div className="text-white/90 text-base font-bold font-['Feather']">
                      {correctVerset}
                    </div>
                  </div>
...
const ClosePartyScreen: React.FC<{
  state: "closed" | "opened";
  onCloseCancel: () => void;
}> = ({ state, onCloseCancel }) => {
...
const EndPartyScreen: React.FC<{ score: number }> = ({ score }) => {
  ...
  return (
    <div
      className={
        "h-screen flex flex-col justify-between w-full px-[20px] bg-[#141f25] absolute top-0 left-0 overflow-hidden"
      }
    >
      <div></div>
      <div className={"flex flex-col gap-[30px] items-center"}>
        {score <= 7 ? (
          <div
            className={
              "text-[64px] text-center flex items-center justify-center"
            }
          >
            😓
          </div>
        ...
        <div>
          <div className="text-[#ffd900] text-[22px] font-bold font-['Feather'] text-center">
        ...
        <div className={"flex gap-3 w-full"}>
          ...
          <div className={"flex-1"}>
            <StatItem
              icon={<DXPIcon width={25} height={25} />}
              title={`${score}`}
              desc={"Score"}
            />
          </div>
...
const StatItem: React.FC<{
  ...
  return (
    <div
      className={
        "h-[73px] w-full px-3.5 py-2.5 bg-transparent rounded-[10px] border-2 border-[#38454e] flex gap-2 items-start"
      }
    >
      <div>{icon}</div>
      <div>
