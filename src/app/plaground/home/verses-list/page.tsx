...
export default function VersetListPage() {
  ...
  return (
    <div>
      ...
      <div
        className={
          "px-[20px] mt-[20px] flex flex-col gap-3 mb-16 max-w-xl mx-auto"
        }
      >
        <NavSection versetCount={myVerses ? myVerses.length : 0} />
        {!training_id && <NewVerset />}
        {loading && <VersesListSkeleton />}
        {myVerses && myVerses.length < 5 && !loading && !training_id && (
          <EmptyVerses verses={myVerses} />
        )}
    ...
    <div className={"flex flex-col gap-3"}>
      {Array.from({ length: 5 }).map((d, i) => (
        <div
          key={i}
          className={
            "w-full h-[50px] bg-bgPrimarySecondary rounded-md animate-pulse"
          }
        ></div>
      ))}
    </div>
...
const HeroSection: React.FC<{ verses?: Verset[] }> = ({ verses }) => {
  ...
  const { categories, loading: loadingCtg } = useCategories({
  ...
  return (
    <div
      className={
        "h-[380px] px-[18px] bg-gradient-to-b from-[#5f3a9a] to-[#1a143a]"
      }
    >
      <div className="max-w-xl mx-auto">
        ...
        <div className={"flex flex-col items-center justify-center gap-[30px]"}>
          ...
          <div className="w-[228px] text-center text-white text-xl font-bold font-['Feather']">
            {curCtg?.name
              ? `${curCtg?.name}`
              : " Entraîne-toi à memoriser les versets par coeur."}
          </div>
...
const VersetCount: React.FC<{ count: number }> = ({ count }) => {
  return (
    <div
      className={
        "w-[47px] h-6 p-0.5 bg-gradient-to-r from-[#2dabbc] to-[#42e1b1] rounded-sm flex-col justify-start items-start gap-2.5 inline-flex overflow-hidden"
      }
    >
      <div className="h-full w-full bg-black rounded-[1px] flex justify-center items-center">
        <div className="text-white text-xl font-bold font-['Feather']">
          {count}
        </div>
      </div>
    ...
    <div>
      ...
      >
        <Button variant={"dashed"} className={"w-full"}>
          {load ? "un instant..." : "Nouveau verset."}
        </Button>
      </Link>
    </div>
  );
